...
const getTooltipMessage = (metricName: string, score: number): string => {
  ...
  if (metricName.includes("Sentinel")) {
    if (score > 0.7)
      return "🔴 Piyasa riskli (VIX yüksek) → BEKLE, işlem yapma";
  ...
  if (metricName.includes("Consensus")) {
    if (score > 0.7) return "✅ Tüm sistem AL diyor → AL";
    if (score < 0.3) return "✅ Tüm sistem SAT diyor → SAT";
    return "⏸️ Sistem kararsız → BEKLE";
  }
          ...
          >
            ...
            {showTooltip && (
              <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 bg-gray-900 border border-cyan-500 rounded-lg p-3 text-xs text-gray-100 z-50 shadow-lg">
                ...
                <p>{tooltipMessage}</p>
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 border-4 border-transparent border-t-cyan-500"></div>
              </div>
            )}
          </div>
        </div>
